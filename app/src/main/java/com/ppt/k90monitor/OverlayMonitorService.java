...
public class OverlayMonitorService extends Service {
    ...
    private String pct(float v) {
        return Float.isNaN(v) ? "N/A" : String.format(Locale.US, "%5.1f%%", v);
    }

    private String freq(float mhz) {
        if (Float.isNaN(mhz)) return "N/A";
        if (mhz >= 1000f) return String.format(Locale.US, "%4.2fGHz", mhz / 1000f);
        return String.format(Locale.US, "%4.0fMHz", mhz);
    }
        ...
        public boolean onTouch(View v, MotionEvent e) {
            switch (e.getActionMasked()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    lp.x = downX + Math.round(dx);
                    lp.y = downY + Math.round(dy);
