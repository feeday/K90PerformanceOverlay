...
同时提供可选的 **红魔散热器遥测扩展** 和 **局域网 FTP 文件传输**。
...
- 不显示小数，四舍五入为整数
- 悬浮窗仍然可以拖动
- 长按关闭功能仍然保留，只是不再显示提示文字
- **某项没有有效数据时，整项直接隐藏，不显示 `--`**
...
5.7 新增内置 **局域网 FTP Server**，可以直接在手机和电脑之间传文件。

FTP 与性能悬浮监控相互独立，不会因为启动悬浮监控而自动启动 FTP，必须在应用设置中手动开启。
